			...
			<span class="user-links <%= themeDisplay.isImpersonated() ? "menu-button": "" %>">
                ...
                <c:choose>
                    <c:when test="<%= group.isControlPanel() %>">
                        <aui:a cssClass="user-portrait" href="<%= themeDisplay.getURLMyAccount().toString() %>" title="manage-my-account">
                            <img alt="<%= HtmlUtil.escape(user.getFullName()) %>" src="<%= HtmlUtil.escape(user.getPortraitURL(themeDisplay)) %>" />
                        </aui:a>

                        <aui:a cssClass="user-fullname" href="<%= themeDisplay.getURLMyAccount().toString() %>" title="manage-my-account"><%= HtmlUtil.escape(user.getFullName()) %></aui:a>
                    </c:when>
                    <c:otherwise>
