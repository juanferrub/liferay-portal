...
if (ddmStructure != null) {
	ddmTemplates.addAll(DDMTemplateServiceUtil.getTemplates(ddmStructureGroupId, PortalUtil.getClassNameId(DDMStructure.class)));
								...
								<div class="journal-form-presentation-label">
									<c:choose>
										...
										<c:when test="<%= ddmTemplates.size() == 1 %>">
											...
											<aui:input name="templateId" type="hidden" value="<%= template.getTemplateKey() %>" />
											...
											<c:if test="<%= DDMTemplatePermission.contains(permissionChecker, template, ActionKeys.UPDATE) %>">
												...
												<portlet:renderURL var="templateURL">
													...
													<portlet:param name="templateId" value="<%= template.getTemplateKey() %>" />
												</portlet:renderURL>
		...
		<div class="journal-article-container" id="<portlet:namespace />journalArticleContainer">
			<c:choose>
				...
				<c:otherwise>
					...
					<%= DDMXSDUtil.getHTML(pageContext, ddmStructure.getXsd(), ddmFields, "", false, locale) %>
				...
				Liferay.Util.openWindow(
					{
						...
						uri: '<portlet:renderURL windowState="<%= LiferayWindowState.POP_UP.toString() %>"><portlet:param name="struts_action" value="/journal/select_template" /><portlet:param name="groupId" value="<%= String.valueOf(groupId) %>" /><portlet:param name="structureId" value="<%= structureId %>" /></portlet:renderURL>'
					}
				);
			}
		);
...
private String _getTemplateImage(ThemeDisplay themeDisplay, DDMTemplate template) {
